...
Tpsi = act(T, psi)
lop = transf_mat(psi, Tpsi)
wr, vr = eigsolve(lop, Tensor(rand, Float64, ℝ^5*ℝ^10), 1)
wr[1]
lop_T = transf_mat_T(psi, Tpsi)
wl, vl = eigsolve(lop_T, Tensor(rand, Float64, ℝ^5*ℝ^10), 1)
wl[1]
vl, vr = vl[1], vr[1]
...
wr, vr = eigsolve(lop, Tensor(rand, Float64, ℝ^5*ℝ^5), 1)
wr, vr = wr[1], vr[1]
vr = permute(vr, (1, ), (2, ))
eigh(vr)


#grad(central_fdm(5,1), psi->real(ovlp(psi, psi)), psi)[1]
# gradient check
for ix in 1:10
    Δpsi = 1e-6*TensorMap(rand, Float64, ℝ^5*ℝ^2, ℝ^5)
    Δy = (real(ovlp(psi+Δpsi, psi+Δpsi)) - real(ovlp(psi-Δpsi, psi-Δpsi))) / 2
    dpsi = gradient(psi->real(ovlp(psi, psi)), psi)[1]
    Δy1 = dot(Δpsi, dpsi)
    print(abs(Δy - Δy1) / abs(Δy) < 1e-8, ' ')
end

function check_act(phi::TensorMap{CartesianSpace, 2, 1})
    Tphi = act(T, phi)
    return real(ovlp(phi, Tphi))
end

for ix in 1:10
    Δpsi = 1e-6*TensorMap(rand, Float64, ℝ^5*ℝ^2, ℝ^5)
    Δy = (check_act(psi+Δpsi) - check_act(psi-Δpsi)) / 2
...
# simple test
psidata = rand(Float64, (4, 2, 4))
psi = data_to_TensorMap(psidata)
reshape(psi.data, (4,2,4))
function check_data_to_TensorMap(psidata::Array{Float64, 3})
    psi = data_to_TensorMap(psidata)
    return real(ovlp(psi, psi))
end
check_data_to_TensorMap(psidata)
gradient(check_data_to_TensorMap, psidata)
gradient(x->real(ovlp(x, x)), psi)

function cost_func(psidata::Array{Float64, 3})
    psi = data_to_TensorMap(psidata)
    Tpsi = act(T, psi)
...
function cost_func_grad!(grad::Array{Float64, 3}, psidata::Array{Float64, 3})
    grad .= gradient(cost_func, psidata)[1]
end

psidata = rand(Float64, (2, 2, 2))
io = open("result.txt", "w+")
for chi in [2, 4, 8, 16, 32, 64, 128] 
    res_f = optimize(cost_func, cost_func_grad!, psidata, LBFGS(), Optim.Options(show_trace=true, iterations=200))
    psidata_final = Optim.minimizer(res_f)
    psi_final = data_to_TensorMap(psidata_final)
